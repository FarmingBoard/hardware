...
bool connectToMQTT()
{
    ...
    if (client.connect("ESP32Client", mqttUser, mqttPass))
    {
        ...
        String request = "{\"sharedKeys\":\"lightState,moistureThreshold,isWatering,timePump,pin_1,pin_2,pin_3\"}";
...
void callback(char *topic, byte *payload, unsigned int length)
{
    ...
    if (!error)
    {
        ...
        if (doc["shared"]["moistureThreshold"])
            moistureThreshold = doc["shared"]["moistureThreshold"];
        if (doc["shared"]["isWatering"])
            isWatering = doc["shared"]["isWatering"];
        if (doc["shared"]["timePump"])
            timePump = doc["shared"]["timePump"];
        if (lightState)
        {
        ...
        {
            int pin_1 = doc["shared"]["pin_1"];
            digitalWrite(RELAY1, pin_1);
        }
        if (doc["shared"].containsKey("pin_2"))
        {
            int pin_2 = doc["shared"]["pin_2"];
            digitalWrite(RELAY2, pin_2);
        }
        if (doc["shared"].containsKey("pin_3"))
        {
            int pin_3 = doc["shared"]["pin_3"];
            digitalWrite(RELAY3, pin_3);
        }
